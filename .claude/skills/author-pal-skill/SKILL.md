...
6. The skill installs to downstream users via the symlink loop in `src/targets/lib.ts` (`copySkills`) on their next `pal cli install`. No manual linking — being present in `assets/skills/` is what ships it.
